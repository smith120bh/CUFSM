function [ curve_distor,shapes_distor] = run_strip_distortional( prop,node,elem,lengths_distor)
%-----------------------------------------------------------------
%----------------additional input definitions---------------------
%-----------------------------------------------------------------
% global curve_distor2 lengths_distor lengthindex_distor
% global lengths_distor lengthindex_distor
% cla(axes_handle)
%Lengths:
%for signiture curve, the length is interpreted as half-wave length the same as older cufsm version (3.x and previous)
% lengths_distor=linspace(len/100,len,30);
...
%-----------------------------------------------------------------
%
[curve_distor,shapes_distor]=strip(prop,node,elem,lengths_distor,springs,constraints,GBTcon,BC,m_all,neigs);
%
%%
% %-----------------------------------------------------------------
...
% nodecell{i}=node;
% elemcell{i}=elem;
% lengthscell{i}=lengths_distor;
% curvecell{i}=curve_distor2;
% shapescell{i}=shapes;
% clascell{i}=clas;
...
% 
% fileindex=1;
% % lengthindex_distor=ceil(length(lengths)/2);
% modeindex=1;
% clasopt=0;
% logopt=1;
% minopt=1;
% files=1;
% modes=(1:1:length(curve_distor2{lengthindex_distor}(:,2)));
% modedisplay=1;
% filedisplay=files;
% 
% %Plot the buckling curve as a start
% curveoption=1; %plot the buckling curve vs length
% if length(lengths_distor)==1
%     curveoption=2;%plot the buckling curve vs mode#
% end
% %set the xmax, ymax
% if curveoption==1
%     xmin=min(lengths_distor)*10/11;
%     ymin=0;
%     xmax=max(lengths_distor)*11/10;
%     for j=1:max(size(curve_distor2));
%         curve_sign(j,1)=curve_distor2{j}(modeindex,1);
%         curve_sign(j,2)=curve_distor2{j}(modeindex,2);
%     end
%     ymax=min([max(curve_sign(:,2)),3*median(curve_sign(:,2))]);
%     
%     %plot the buckling curve vs length
%     picpoint=[curve_distor2{lengthindex_distor}(modeindex,1) curve_distor2{lengthindex_distor}(modeindex,2)];
%     [distor_len]=thecurve3_hole(curvecell,filenamecell,clascell,filedisplay,minopt,logopt,clasopt,axescurve,xmin,xmax,ymin,ymax,modedisplay,fileindex,modeindex,picpoint)
%     
...
%     xmin=1;
%     ymin=0;
%     xmax=length(curve_distor2{lengthindex_distor}(:,2));
%     ymax=min([max(curve_distor2{lengthindex_distor}(:,2)),3*median(curve_distor2{lengthindex_distor}(:,2))]);
%     %plot the buckling load vs mode#
%     picpoint=[modes(modeindex) curve_distor2{lengthindex_distor}(modeindex,2)];
%     thecurve3mode(curvecell,filenamecell,clascell,fileindex,minopt,logopt,clasopt,axescurvemode,xmin,xmax,ymin,ymax,fileindex,lengthindex_distor,picpoint);
%    
%     %plot longitudinal term participation for modeindex
%     figure(12)
%     set(gca,'FontName','times','FontSize',14)
%     
%     mode=shapes{lengthindex_distor}(:,modeindex);
%     nnodes=length(node(:,1));
%     m_a=m_all{lengthindex_distor};
%     [d_part]=longtermpart(nnodes,mode,m_a);
%     bar(m_a,d_part);hold on
%     xlabel('m, longitudinal term');hold on;
%     ylabel('Participation');hold on;
%     legendstring=[filenamecell{fileindex},', length = ',num2str(lengths_distor(lengthindex_distor)), ', mode = ', num2str(modeindex)];
...
%     dispshp2(undef,lengths_distor(lengthindex_distor),node,elem,mode,axes3dshape,scale,m_a,BC,ifpatch);
% end
