function [xc,yc,Ix,Iy,Ixy,theta,I1,I2,J,xs,ys,Cw,B1,B2,Pe,dcoord] = cutwp_prop_4hole(node, elem,KL1,KL2,KL3,prop,props4global,force,c)
...
% global     Ix_avg Iy_avg I1_avg I2_avg Cw_avg J_avg
...
%         x1 = coord12(sn,1); y1 = coord12(sn,2);
%         x2 = coord12(fn,1); y2 = coord12(fn,2);
%         B1 = B1+((y1+y2)*(y1^2+y2^2)/4+(y1*(2*x1^2+(x1+x2)^2)+y2*(2*x2^2+(x1+x2)^2))/12)*L(i);
%         B2 = B2+((x1+x2)*(x1^2+x2^2)/4+(x1*(2*y1^2+(y1+y2)^2)+x2*(2*y2^2+(y1+y2)^2))/12)*L(i);
%     end
%     B1 = B1/I1-2*s12(2);
%     B2 = B2/I2-2*s12(1); 
%     
...
%     end
    ...
    %%
    Ix_avg=props4global.Ixx;
Iy_avg=props4global.Izz;
I1_avg=props4global.I11;
I2_avg=props4global.I22;
Cw_min=props4global.Cw;
J_avg=props4global.J;
A=props4global.A;
...
%     F=pi/KL1*sqrt(E*I1*(G*J+pi^2*E*Cw/KL1^2));
%      assignin('base', 'Pe3', Pe3);
%       assignin('base', 'F', F);
...
% Pcr1=pi/KL2*sqrt(E*I2*(G*J+pi^2*E*Cw/KL2^2));
% Pcr2=-pi/KL2*sqrt(E*I2*(G*J+pi^2*E*Cw/KL2^2));
%  assignin('base', 'KL2q', KL2);
%  assignin('base', 'I2q', I2);
%  assignin('base', 'Jq', J);
%  assignin('base', 'Cwq', Cw);
%   assignin('base', 'Eq', E);
%    assignin('base', 'Gq', G);
